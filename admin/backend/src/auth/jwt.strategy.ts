...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: any) {
    // Always fetch fresh permissions from DB so admin changes take effect immediately
    const user = await this.userRepository.findOne({ where: { id: payload.id || payload.sub } });
    return {
